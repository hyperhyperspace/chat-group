import { CapabilitySet, GrantOp, Hash, HashedObject, HashReference, Identity, MutationOp, RevokeAfterOp, UseOp } from '@hyper-hyper-space/core';

class UserSet extends CapabilitySet {

    static className = 'chat-group/v0/model/UserSet';
    ...
    private isAdminOp(op: GrantOp|RevokeAfterOp, opReferences: Map<Hash, HashedObject>, admin?: Identity): boolean {
        ...
        if (causalUseOp.usageKey !== UserSet.getGranteeCapabilityKeyForOp(op)) {
            return false;
        }
    ...
    addUser(id: Identity, admin?: Identity): Promise<boolean> {
        ...
        if (userOp === undefined) {
            if (admin === undefined) {
                ...
                const usageKey = UserSet.getGranteeCapabilityKey(id, 'user', false);
    ...
    removeUser(id: Identity, admin?: Identity): Promise<boolean> {
        ...
        if (admin !== undefined && allValidGrants.size > 0) {
            const usageKey = UserSet.getGranteeCapabilityKey(id, 'user', true);
            const useAdminOp = this.useCapability(admin, 'admin', usageKey);
...
HashedObject.registerClass(UserSet.className, UserSet);
